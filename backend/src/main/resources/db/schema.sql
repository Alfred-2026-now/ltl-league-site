...
CREATE TABLE IF NOT EXISTS `matches` (
  ...
  `schedule_published` TINYINT NOT NULL DEFAULT 0 COMMENT '赛程是否发布（0:未发布 1:已发布）',
  `schedule_published_at` DATETIME NULL COMMENT '赛程发布时间',
  `schedule_unpublished_at` DATETIME NULL COMMENT '赛程撤回时间',
  ...
  UNIQUE KEY `uk_season_round_teams` (`season`, `round`, `home_team_id`, `away_team_id`, `deleted`),
...
CREATE TABLE IF NOT EXISTS `match_results` (
  ...
  `home_line_value` DECIMAL(12,2) UNSIGNED NULL COMMENT '主队按出场局数加权的总身价',
  `away_line_value` DECIMAL(12,2) UNSIGNED NULL COMMENT '客队按出场局数加权的总身价',
...
-- 11. 赛事任务系统
-- 已有数据库请执行 migration_event_tasks.sql；以下定义用于全新建库参考。
CREATE TABLE IF NOT EXISTS `event_tasks` (
  ...
  `p_reward` INT UNSIGNED NOT NULL DEFAULT 0,
  `bounty_reward` INT UNSIGNED NOT NULL DEFAULT 0,
  `budget_note` VARCHAR(500) NULL,
  `anonymous_fee_rate_snapshot` INT UNSIGNED NULL,
  `anonymous_fee_amount` INT UNSIGNED NOT NULL DEFAULT 0,
...
CREATE TABLE IF NOT EXISTS `event_task_reviews` (
  `id` BIGINT UNSIGNED NOT NULL AUTO_INCREMENT,
...
CREATE TABLE IF NOT EXISTS `event_task_claims` (
  `id` BIGINT UNSIGNED NOT NULL AUTO_INCREMENT,
  `task_id` BIGINT UNSIGNED NOT NULL,
  ...
  `admin_cancel_reason` VARCHAR(500) NULL,
  `created_at` DATETIME NOT NULL DEFAULT CURRENT_TIMESTAMP,
  `updated_at` DATETIME NOT NULL DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP,
  `deleted` TINYINT NOT NULL DEFAULT 0,
  PRIMARY KEY (`id`),
  KEY `idx_event_task_claim_player_history` (`task_id`, `player_id`, `id`),
...
CREATE TABLE IF NOT EXISTS `event_task_proofs` (
  `id` BIGINT UNSIGNED NOT NULL AUTO_INCREMENT,
  `task_id` BIGINT UNSIGNED NOT NULL,
...
CREATE TABLE IF NOT EXISTS `event_task_proof_images` (
  `id` BIGINT UNSIGNED NOT NULL AUTO_INCREMENT,
...
CREATE TABLE IF NOT EXISTS `player_bounty_ledger` (
  `id` BIGINT UNSIGNED NOT NULL AUTO_INCREMENT,
  `player_id` BIGINT UNSIGNED NOT NULL,
  `season` VARCHAR(20) NOT NULL,
